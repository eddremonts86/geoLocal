import { bigserial, boolean, doublePrecision, index, integer, jsonb, pgEnum, pgTable, primaryKey, text, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
var scrapingSourceStatusEnum = pgEnum("scraping_source_status", [
	"active",
	"paused",
	"deprecated"
]);
var scrapingSourceKindEnum = pgEnum("scraping_source_kind", [
	"built-in",
	"external",
	"none"
]);
var scrapingSources = pgTable("scraping_sources", {
	...
	notes: text("notes"),
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => [index("idx_scraping_sources_status").on(table.status), index("idx_scraping_sources_kind").on(table.kind)]);
var scrapedRaw = pgTable("scraped_raw", {
	id: uuid("id").defaultRandom().primaryKey(),
	source: varchar("source", { length: 64 }).notNull().references(() => scrapingSources.key, {
		onUpdate: "cascade",
		onDelete: "restrict"
	}),
...
var scrapedSourceCandidateStatusEnum = pgEnum("scraped_source_candidate_status", [
	"pending",
	"approved",
	"rejected",
	"dead"
]);
var scrapedSourceCandidates = pgTable("scraped_source_candidates", {
	id: uuid("id").defaultRandom().primaryKey(),
...
}, (t) => [index("idx_threads_listing").on(t.listingId), index("idx_threads_last_message").on(t.lastMessageAt)]);
var threadParticipants = pgTable("thread_participants", {
...
}, (t) => [primaryKey({ columns: [t.threadId, t.userId] }), index("idx_thread_participants_user").on(t.userId)]);
...
}, (t) => [index("idx_messages_thread_created").on(t.threadId, t.createdAt), index("idx_messages_sender").on(t.senderId)]);
var notifications = pgTable("notifications", {
	...
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (t) => [index("idx_notifications_user_unread").on(t.userId, t.readAt), index("idx_notifications_user_created").on(t.userId, t.createdAt)]);
var listingReports = pgTable("listing_reports", {
	id: uuid("id").defaultRandom().primaryKey(),
	listingId: uuid("listing_id").notNull().references(() => listings.id, { onDelete: "cascade" }),
	...
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (t) => [index("idx_listing_reports_listing").on(t.listingId), index("idx_listing_reports_resolved").on(t.resolvedAt)]);
var rateLimitBuckets = pgTable("rate_limit_buckets", {
	bucketKey: text("bucket_key").notNull(),
	windowStart: timestamp("window_start").notNull(),
	count: integer("count").notNull().default(0)
}, (t) => [primaryKey({ columns: [t.bucketKey, t.windowStart] }), index("idx_rate_limit_buckets_window").on(t.windowStart)]);
...
}, (t) => [index("idx_stripe_events_type").on(t.type), index("idx_stripe_events_processed").on(t.processedAt)]);
pgTable("listing_views", {
...
//#endregion
export { scrapingSources as _, listingProperties as a, threads as b, listingTranslations as c, messages as d, notifications as f, scrapedSourceCandidates as g, scrapedRaw as h, listingFeatures as i, listingVehicles as l, rateLimitBuckets as m, listingAssets as n, listingReports as o, payments as p, listingExperiences as r, listingServices as s, favorites as t, listings as u, stripeEvents as v, userProfiles as x, threadParticipants as y };
