 ...
 */
export function haversineCondition(lat: number, lng: number, radiusKm: number): SQL {
  // Bounding box delta (conservative — 1° lat ≈ 111km, 1° lng ≈ 111km·cos(lat))
  const latDelta = radiusKm / 111
  const lngDelta = radiusKm / (111 * Math.max(Math.cos((lat * Math.PI) / 180), 0.01))

  const bbox = and(
    gte(listings.latitude, lat - latDelta),
    lte(listings.latitude, lat + latDelta),
    gte(listings.longitude, lng - lngDelta),
    lte(listings.longitude, lng + lngDelta),
  )!

  const haversine = sql`${EARTH_RADIUS_KM} * acos(
...
export function polygonBboxCondition(poly: PolygonRing): SQL {
 ...
 */
export function pointInPolygon(lng: number, lat: number, poly: PolygonRing): boolean {
  let inside = false
 ...
 */
export function encodePolygon(poly: PolygonRing): string {
...
export function decodePolygon(str: string | null | undefined): PolygonRing | null {
  if (!str) return null
