...
var import_browser_external_os = /* @__PURE__ */ __toESM(require_browser_external_os(), 1);
var import_browser_external_fs = /* @__PURE__ */ __toESM(require_browser_external_fs(), 1);
...
//#endregion
//#region browser-external:perf_hooks
var require_browser_external_perf_hooks = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = Object.create(new Proxy({}, { get(_, key) {
		if (key !== "__esModule" && key !== "__proto__" && key !== "constructor" && key !== "splice") console.warn(`Module "perf_hooks" has been externalized for browser compatibility. Cannot access "perf_hooks.${key}" in client code. See https://vite.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`);
	} }));
}));
//#endregion
//#region node_modules/.pnpm/postgres@3.4.9/node_modules/postgres/src/result.js
var import_browser_external_net = /* @__PURE__ */ __toESM(require_browser_external_net(), 1);
var import_browser_external_tls = /* @__PURE__ */ __toESM(require_browser_external_tls(), 1);
var import_browser_external_crypto = /* @__PURE__ */ __toESM(require_browser_external_crypto(), 1);
var import_browser_external_stream = /* @__PURE__ */ __toESM(require_browser_external_stream(), 1);
var import_browser_external_perf_hooks = require_browser_external_perf_hooks();
...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
	...
	async function createSocket() {
		let x;
		try {
			x = options.socket ? await Promise.resolve(options.socket(options)) : new import_browser_external_net.default.Socket();
		} catch (e) {
			error(e);
			return;
		}
	...
	async function secure() {
		...
		const options = {
			socket,
			servername: import_browser_external_net.default.isIP(socket.host) ? void 0 : socket.host
		};
		...
		socket = import_browser_external_tls.default.connect(options);
	...
	function reconnect() {
		setTimeout(connect, closedTime ? Math.max(0, closedTime + delay - import_browser_external_perf_hooks.performance.now()) : 0);
	}
	function connected() {
		try {
			statements = {};
	...
	async function closed(hadError) {
		...
		closedTime = import_browser_external_perf_hooks.performance.now();
...
UnknownMessage)(xs);
	...
	async function SASL() {
		nonce = (await import_browser_external_crypto.default.randomBytes(18)).toString("base64");
	...
	async function SASLContinue(x) {
		...
		const saltedPassword = await import_browser_external_crypto.default.pbkdf2Sync(await Pass(), Buffer.from(res.s, "base64"), parseInt(res.i), 32, "sha256");
	...
	function CopyInResponse() {
		stream = new import_browser_external_stream.default.Writable({
			autoDestroy: true,
	...
	function CopyOutResponse() {
		stream = new import_browser_external_stream.default.Readable({ read() {
			socket.resume();
		} });
		query.resolve(stream);
	}
	...
	function CopyBothResponse() {
		stream = new import_browser_external_stream.default.Duplex({
			autoDestroy: true,
			read() {
				socket.resume();
			},
...
function md5(x) {
	return import_browser_external_crypto.default.createHash("md5").update(x).digest("hex");
}
function hmac(key, x) {
	return import_browser_external_crypto.default.createHmac("sha256", key).update(x).digest();
}
function sha256(x) {
	return import_browser_external_crypto.default.createHash("sha256").update(x).digest();
}
function xor(a, b) {
	...
	return new Promise(async (resolve, reject) => {
        ...
        `
			...
			async function readable({ highWaterMark = 2048 * 8, start = 0, end = Infinity } = {}) {
				...
				return new import_browser_external_stream.default.Readable({
					highWaterMark,
					async read(size) {
			...
			async function writable({ highWaterMark = 2048 * 8, start = 0 } = {}) {
				start && await lo.seek(start);
				return new import_browser_external_stream.default.Writable({
					highWaterMark,
					write(chunk, encoding, callback) {
	...
	function Sql(handler) {
		...
		function file(path, args = [], options = {}) {
			...
			return new Query([], args, (query) => {
				import_browser_external_fs.default.readFile(path, "utf8", (err, string) => {
...
function osUsername() {
	try {
		return import_browser_external_os.default.userInfo().username;
	} catch (_) {
...
//# sourceMappingURL=src-DV6TwKGB.js.map