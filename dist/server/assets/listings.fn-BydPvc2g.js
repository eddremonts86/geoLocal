...
import { a as listingProperties, c as listingTranslations, i as listingFeatures, l as listingVehicles, n as listingAssets, r as listingExperiences, s as listingServices, u as listings } from "./schema-Bm7YGE-a.js";
import { a as polygonBboxCondition, i as pointInPolygon, r as haversineCondition, t as decodePolygon } from "./spatial-ChWRs6lm.js";
import { z } from "zod";
	...
	if (experienceIds.length > 0) {
		...
		for (const e of expRows) {
			...
			if (data.languages?.length) {
				if (!(e.languages ?? []).some((l) => data.languages.includes(l))) include = false;
			}
...
var getListingBySlugFn = createServerFn({ method: "GET" }).inputValidator(z.object({
	...
	const db = await loadDb();
	const [listing] = await db.select().from(listings).where(and(eq(listings.slug, data.slug), eq(listings.status, "published"), eq(listings.moderationStatus, "ok"))).limit(1);
	...
	return {
		...
		contactMethod: listing.contactMethod ?? null,
...
var getSimilarListingsFn_createServerFn_handler = createServerRpc({
	id: "ae2e7a6177a9d0793b828af0d07977229d54b38b6469697b8b9a710bcbab9e5b",
	name: "getSimilarListingsFn",
	filename: "src/modules/listings/api/listings.fn.ts"
}, (opts) => getSimilarListingsFn.__executeServer(opts));
var getSimilarListingsFn = createServerFn({ method: "GET" }).inputValidator(z.object({
	...
	locale: z.string().default("en")
})).handler(getSimilarListingsFn_createServerFn_handler, async ({ data }) => {
	const db = await loadDb();
	...
	let rows = await db.select().from(listings).where(and(...sameCityConditions)).orderBy(sql`${listings.featured} desc, ${listings.publishedAt} desc`).limit(data.limit);
	if (rows.length < data.limit) {
		const extra = await db.select().from(listings).where(and(eq(listings.status, "published"), eq(listings.category, data.category), sql`${listings.id} != ${data.excludeId}`, sql`${listings.id} NOT IN (${sql.join(rows.length ? rows.map((r) => sql`${r.id}`) : [sql`''`], sql`, `)})`)).orderBy(sql`${listings.featured} desc, ${listings.publishedAt} desc`).limit(data.limit - rows.length);
		rows = [...rows, ...extra];
	}
	if (rows.length === 0) return [];
	const ids = rows.map((r) => r.id);
	const translations = await db.select().from(listingTranslations).where(and(inArray(listingTranslations.listingId, ids), eq(listingTranslations.locale, data.locale)));
	const fallback = data.locale !== "en" ? await db.select().from(listingTranslations).where(and(inArray(listingTranslations.listingId, ids), eq(listingTranslations.locale, "en"))) : [];
	const covers = await db.select().from(listingAssets).where(and(inArray(listingAssets.listingId, ids), eq(listingAssets.isCover, true)));
	const tMap = new Map(translations.map((t) => [t.listingId, t]));
...
var getMapMarkersFn = createServerFn({ method: "GET" }).inputValidator(mapMarkersInputSchema).handler(getMapMarkersFn_createServerFn_handler, async ({ data }) => {
	...
	const needsPropertyFilter = data.bedrooms?.length || data.bathrooms != null || data.areaMin != null || data.areaMax != null || data.yearBuiltMin != null || data.yearBuiltMax != null || data.parkingMin != null || data.furnished === true;
	const needsVehicleFilter = data.make?.length || data.yearMin != null || data.yearMax != null || data.fuelType?.length || data.transmission || data.mileageMax != null || data.doorsMin != null || data.colors?.length;
	const needsServiceFilter = data.experienceMin != null || data.serviceRadiusMin != null || data.responseTime || data.certified === true;
	const needsExperienceFilter = data.durationMin != null || data.durationMax != null || data.groupMax != null || data.minAgeMax != null || data.difficulty || data.languages?.length;
...
var getHomeStatsFn_createServerFn_handler = createServerRpc({
	id: "80aaeedbd827930c04411db74c2c742359d4e8c80ca03e419c5b652431b55331",
	name: "getHomeStatsFn",
	filename: "src/modules/listings/api/listings.fn.ts"
}, (opts) => getHomeStatsFn.__executeServer(opts));
var getHomeStatsFn = createServerFn({ method: "GET" }).inputValidator(z.object({
	...
	locale: z.string().default("en")
})).handler(getHomeStatsFn_createServerFn_handler, async ({ data }) => {
	const db = await loadDb();
	const catRows = await db.select({
		category: listings.category,
		count: sql`count(*)::int`
	}).from(listings).where(eq(listings.status, "published")).groupBy(listings.category);
	...
	const neighRows = await db.select({
		city: listings.city,
		region: listings.region,
		count: sql`count(*)::int`
	}).from(listings).where(eq(listings.status, "published")).groupBy(listings.city, listings.region).orderBy(sql`count(*) desc`).limit(data.neighborhoodLimit * 3);
	...
	let heroMarkers = await db.select({
		id: listings.id,
		slug: listings.slug,
		category: listings.category,
		price: listings.price,
		...
		latitude: listings.latitude,
		longitude: listings.longitude
	}).from(listings).where(and(eq(listings.status, "published"), eq(listings.featured, true))).orderBy(sql`RANDOM()`).limit(data.heroMarkersLimit);
	if (heroMarkers.length < data.heroMarkersLimit) {
		const extra = await db.select({
			...
			latitude: listings.latitude,
			longitude: listings.longitude
		}).from(listings).where(eq(listings.status, "published")).orderBy(sql`RANDOM()`).limit(data.heroMarkersLimit - heroMarkers.length);
	...
	for (const cat of verticals) {
		let picked = await db.select({
			id: listings.id,
			slug: listings.slug
		}).from(listings).where(and(eq(listings.status, "published"), eq(listings.category, cat), eq(listings.featured, true))).orderBy(sql`RANDOM()`).limit(data.samplesPerVertical * 2);
		if (picked.length < data.samplesPerVertical) {
			const extra = await db.select({
				id: listings.id,
				slug: listings.slug
			}).from(listings).where(and(eq(listings.status, "published"), eq(listings.category, cat))).orderBy(sql`RANDOM()`).limit(data.samplesPerVertical * 2);
			picked = [...picked, ...extra];
		}
...
//#endregion
export { getFeaturedListingsFn_createServerFn_handler, getHomeStatsFn_createServerFn_handler, getListingBySlugFn_createServerFn_handler, getMapMarkersFn_createServerFn_handler, getScrapedListingsFn_createServerFn_handler, getSimilarListingsFn_createServerFn_handler, searchListingsFn_createServerFn_handler };
